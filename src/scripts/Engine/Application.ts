...
export class Application {
    ...
    public init(): void {
        const loadingUI = new LoadingUI(this.loader.resources[Assets.LoadingJson]?.data.loading);
        const basicUi = new BasicUI(this.loader.resources[ Assets.PerformanceJSON]?.data.performance);
        this.stage.addChild(loadingUI);
        this.stage.addChild(basicUi);
        this.scheduleTaskOnce(3, () => {
            this.stage.removeChild(loadingUI);
            const mainScene: MainScene = new MainScene(this.loader.resources[ Assets.MainJSON]?.data.mainScene);
