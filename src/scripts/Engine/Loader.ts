...
export class Loader {
    ...
    private onManifestLoaded(_loader: PIXI.Loader, resources: Partial<Record<string, PIXI.LoaderResource>>): void {
        const resourceArray = resources[Assets.MainManifset]?.data.preLoadresources;
        this.preLoad(resourceArray);
    }

    /*  Call function necessary to start preLoading */
    private preLoad(resourceArray: []): void {
        this.addFileNeededToLoad(resourceArray);
        this.loadAddedFile();
    }

    /*  Traverse the `manifest.json` and add all preload media needed to load */
    private addFileNeededToLoad(resourceArray: []): void {
        resourceArray.forEach((element: string) => {
            game.loader.add(element);
        });
    }
